...
if ( preg_match( "#^wordpress-firewall-2.php#", basename( $_SERVER['PHP_SELF'] ) ) ) {
	exit();
}
...
add_option( 'WP_firewall_whitelisted_ip',
	serialize(
		array(
			'0' => stripslashes( $_SERVER['REMOTE_ADDR'] )
		)
	)
);
add_option( 'WP_firewall_whitelisted_page', '' );
add_option( 'WP_firewall_whitelisted_variable', '' );
add_option( 'WP_firewall_plugin_url',
	get_option( 'siteurl' ) . '/wp-admin/options-general.php?page=' . basename( __FILE__ ) );
				...
				foreach ( $request_string as $key => $value ) {
					if ( preg_match( $preg, $value ) ) {
						if ( ! WP_firewall_check_ip_whitelist() ) {
							WP_firewall_send_log_message( $key, $value, 'directory-traversal-attack',
								'Directory Traversal' );
		...
		if ( get_option( 'WP_firewall_exclude_terms' ) == 'allow' ) {
			...
			foreach ( $exclude_terms as $preg ) {
				foreach ( $request_string as $key => $value ) {
					if ( preg_match( $preg, $value ) ) {
						if ( ! WP_firewall_check_ip_whitelist() ) {
							WP_firewall_send_log_message( $key, $value, 'wp-specific-sql-injection-attack',
			...
			foreach ( $_FILES as $file ) {
				...
				foreach ( $file_extensions as $regex ) {
					if ( preg_match( $regex, $file['name'] ) ) {
						...
						WP_firewall_send_log_message( '$_FILE', $file['name'], 'executable-file-upload-attack',
		...
		if ( get_option( 'WP_firewall_exclude_http' ) == 'allow' ) {
			...
			foreach ( $exclude_terms as $preg ) {
				foreach ( $request_string as $key => $value ) {
					if ( preg_match( $preg, $value ) ) {
						if ( ! WP_firewall_check_ip_whitelist() ) {
							WP_firewall_send_log_message( $key, $value, 'remote-file-execution-attack',
...
function add_settings_link( $links, $file ) {
	...
	if ( $file == $this_plugin ) {
		$settings_link = '<a href="options-general.php?page=' . $this_plugin . '">' . __( "Settings",
				"wordpress-firewall-2" ) . '</a>';
...
function WP_firewall_submenu() {
	...
	if ( !empty( $_REQUEST['set_exclusions'] )
		|| !empty( $_REQUEST['turn_off_email'] )
		|| !empty( $_REQUEST['set_whitelist_variable'] )
		|| !empty( $_REQUEST['set_email'] )
		|| !empty( $_REQUEST['set_whitelist_ip'] )
		|| !empty( $_REQUEST['set_whitelist_page'] )
		|| !empty( $_REQUEST['suppress'] ) ) {

		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}

		check_admin_referer('wordpress-firewall-2');

		if ( $_REQUEST['set_exclusions'] ) {

			update_option( 'WP_firewall_redirect_page', isset( $_REQUEST['redirect_type'] ) );
			update_option( 'WP_firewall_exclude_directory',  isset( $_REQUEST['block_directory'] ) );
			update_option( 'WP_firewall_exclude_queries',  isset( $_REQUEST['block_queries'] ) );
			update_option( 'WP_firewall_exclude_terms',  isset( $_REQUEST['block_terms'] ) );
			update_option( 'WP_firewall_exclude_spaces',  isset( $_REQUEST['block_spaces'] ) );
			update_option( 'WP_firewall_exclude_file',  isset( $_REQUEST['block_file'] ) );
			update_option( 'WP_firewall_exclude_http',  isset( $_REQUEST['block_http'] ) );
			...
			$action_url = str_replace( '&turn_off_email=1', '', esc_url( sanitize_text_field( $_SERVER['REQUEST_URI'] ) ) );
			echo '<div class="updated fade"><p>Emails are now turned off.</p></div>';

		} elseif ( $_REQUEST['set_whitelist_variable'] ) {
			echo '<div class="updated fade"><p>Whitelisted ' . sanitize_text_field( $_REQUEST['set_whitelist_variable'] ). '</p></div>';
			$pages       = unserialize( get_option( 'WP_firewall_whitelisted_page' ) );
			$variables   = unserialize( get_option( 'WP_firewall_whitelisted_variable' ) );
			$pages[]     = '';
			$variables[] = sanitize_text_field( $_REQUEST['set_whitelist_variable'] );
			...
			$action_url = str_replace( ( '&set_whitelist_variable=' . sanitize_text_field( $_REQUEST['set_whitelist_variable'] ) ), '',
			...
			update_option( 'WP_firewall_email_address', sanitize_email( $_REQUEST['email_address'] ) );
			update_option( 'WP_firewall_email_limit', sanitize_text_field( $_REQUEST['email_limit'] ) );
			update_option( 'WP_firewall_email_type', sanitize_text_field( $_REQUEST['email_type'] ) );
			echo '<div class="updated fade"><p>Email settings updated.</p></div>';

		} elseif ( $_REQUEST['set_whitelist_ip'] ) {
			update_option( 'WP_firewall_whitelisted_ip', serialize( sanitize_text_field( $_REQUEST['whitelisted_ip'] ) ) );
			echo '<div class="updated fade"><p>Whitelisted IP set.</p></div>';

		} elseif ( $_REQUEST['set_whitelist_page'] ) {
			update_option( 'WP_firewall_whitelisted_page', serialize( sanitize_text_field( $_REQUEST['whitelist_page'] ) ) );
			update_option( 'WP_firewall_whitelisted_variable', serialize( sanitize_text_field( $_REQUEST['whitelist_variable'] ) ) );
			...
			$action_url = str_replace( '&suppress=0', '', ssanitize_text_field( $_SERVER['REQUEST_URI'] ) );
		}
	}
	?>
	<div class="wrap">
		...
		<form name="set-exclusion-options" action="<?php echo esc_url( $action_url ); ?>" method="post" class="widefat"
		      style="padding:0 0 20px; margin:20px 0 0;">

			<?php wp_nonce_field('wordpress-firewall-2'); ?>

			<div style="padding:0 20px;">
				<h3>Apply Security Filters:</h3>
				<p><input type="checkbox" value="allow"
				          name="block_directory" <?php echo ( get_option( 'WP_firewall_exclude_directory' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block directory traversals (../, ../../etc/passwd, etc.) in application parameters.</p>
				<p><input type="checkbox" value="allow"
				          name="block_queries" <?php echo ( get_option( 'WP_firewall_exclude_queries' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block SQL queries (union select, concat(, /**/, etc.) in application parameters.</p>
				<p><input type="checkbox" value="allow"
				          name="block_terms" <?php echo ( get_option( 'WP_firewall_exclude_terms' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block WordPress specific terms (wp_, user_login, etc.) in application parameters.</p>
				<p><input type="checkbox" value="allow"
				          name="block_spaces" <?php echo ( get_option( 'WP_firewall_exclude_spaces' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block field truncation attacks in application parameters.</p>
				<p><input type="checkbox" value="allow"
				          name="block_file" <?php echo ( get_option( 'WP_firewall_exclude_file' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block executable file uploads (.php, .exe, etc.)</p>
				<p><input type="checkbox" value="allow"
				          name="block_http" <?php echo ( get_option( 'WP_firewall_exclude_http' ) == 'allow' ) ? 'checked="checked"' : '' ?> />
					Block leading http:// and https:// in application parameters (<em>off</em> by default; may cause
					problems with many plugins).</p>
				...
				<table border="0" cellpadding="0" cellspacing="0" style="width:260px; margin-top:0; padding:0;">
					<tr>
						...
						<td><input type="radio" name="redirect_type"
						           value="404page" <?php echo ( get_option( 'WP_firewall_redirect_page' ) == '404page' ) ? 'checked="checked"' : '' ?> />
						</td>
					</tr>
					<tr>
						<td><strong>Redirect To Homepage:</strong></td>
						<td><input type="radio" name="redirect_type"
						           value="homepage" <?php echo ( get_option( 'WP_firewall_redirect_page' ) == 'homepage' ) ? 'checked="checked"' : '' ?> />
						</td>
					</tr>
				</table>
				<p style="margin-top:5px;">
					<small><em>Note: All filters are subject to "Whitelisted IPs" and "Whitelisted Pages" below.</em>
					</small>
				</p>
		...
		<form name="email_address" action="<?php echo esc_url( $action_url ); ?>" method="post" class="widefat"
		      style="padding:0 0 20px; margin:20px 0 0;">
			...
			<div style="padding:0 20px;">
				...
				<input type="text" value="<?php echo esc_html( get_option( 'WP_firewall_email_address' ) ) ?>"
				       name="email_address"/>
				<p style="margin-top:5px;">
					<small><em>Note: Leave this setting blank to disable emails.</em></small>
				</p>
				<p><strong>Email type:</strong> <input type="radio" name="email_type"
                           value="html"<?php echo ( get_option( 'WP_firewall_email_type' ) == 'html' ) ? 'checked="checked"' : '' ?> />html
					<input type="radio" name="email_type"
					       value="text" <?php echo ( get_option( 'WP_firewall_email_type' ) == 'text' ) ? 'checked="checked"' : '' ?> />text
				</p>
				<p><strong>Suppress similar attack warning emails:</strong> <input type="radio" name="email_limit"
                           value="on"<?php echo ( get_option( 'WP_firewall_email_limit' ) == 'on' ) ? 'checked="checked"' : '' ?> />On
					<input type="radio" name="email_limit"
					       value="off" <?php echo ( get_option( 'WP_firewall_email_limit' ) == 'off' ) ? 'checked="checked"' : '' ?> />Off
				</p>
				<input type="submit" name="set_email" value="Set Email" class="button-secondary"/>
			</div>
		</form>

		<form name="whitelist_ip" action="<?php echo esc_url( $action_url ); ?>" method="post" class="widefat"
		      style="padding:0 0 20px; margin:20px 0 0;">
			...
			<div style="padding:0 20px;">
				...
				if ( ! get_option( 'WP_firewall_whitelisted_ip' ) ) {
					...
					if ( $ips_options_unserialized !== false ) {
						...
						foreach ( $ips as $ip ) {
							if ( $ip != '' ) {
								echo '<input type="text" value="' . esc_html( $ip ) . '" name="whitelisted_ip[]" /><br />';
							}
						}
					} else {
				...
				?>
				<p style="margin-top:5px;">
					<small><em>Note: Set field(s) to blank to disable IP whitelist. Your current IP is:
							<strong><?php echo esc_attr( $_SERVER['REMOTE_ADDR'] ) ?></strong>.</em></small>
				</p>
				<input type="submit" name="set_whitelist_ip" value="Set Whitelisted IPs" class="button-secondary"/>
			</div>
		</form>

		<form name="whitelist_page_or_variable" action="<?php echo esc_url( $action_url ); ?>" method="post" class="widefat"
		      style="padding:0 0 20px; margin:20px 0 0;">
			...
			<div style="padding:0 20px;">
						...
						if ( ( $whitelist_pages_unserialized !== false ) && ( $whitelist_pages_unserialized !== false ) ) {
							...
							while ( $count < sizeof( $pages ) ) {
								if ( ( $pages[ $count ] != '' ) || ( $variables[ $count ] != '' ) ) {
									echo '<tr><td><input type="text" value="' . esc_html( $pages[ $count ] ) . '" name="whitelist_page[]" /></td>';
									echo '<td><input type="text" value="' . esc_html( $variables[ $count ] ) . '" name="whitelist_variable[]" /></td></tr>';
								}
								$count ++;
							}
						} else {
				...
				<p style="margin-top:5px;">
					<small><em>Note: Set field(s) to blank to disable page whitelist.<br/>Note: Use *'s for wildcard
							characters.</em></small>
				</p>
...
function WP_firewall_show_plugin_link() { ?>
	<div style="margin:30px 0 20px; text-align:right;">
		<small>Modifications to this plugin by <a href="http://matthewpavkov.com" target="_blank">Matthew
				Pavkov</a>.<br/>Please use the <a href="http://wordpress.org/tags/wordpress-firewall-2?forum_id=10"
		                                          target="_blank">Wordpress Plugin Forum</a> to report bugs,
			suggestions, etc.
		</small>
		<br/><br/>
